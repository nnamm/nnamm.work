...
さて、Pop!\_Os で C/C++環境 + neovim with Packer + LSP を整えたところ、思いのほか手間がかかりました。

コードは書けるものの、clangd が iostream など一部のインクルードパスを認識してくれないんですよね。macOS では問題ないため、設定や環境依存のような気もしますが追求はやめました。疲れた。。。

あと LSP が思ったほど便利に機能していません。C/C++初学者の僕にとって LSP のサポートが弱いのはちょっとキツイ。

そこで、すでに PyCharm / GoLand を使っている僕が CLion を導入しようと決めたのがコトのはじまり。
...
ちなみに CLion のサポート機能はすごく良いですね。

前述のインクルードパスは問題ありませんし、ほどよく補完候補が出てきます。また開発のお作法（Makefile を使ったり、CMakeLists.txt を使ったりするんですよね）とか、やった方がいいことなどを示唆してくれます。

警告やエラーも同じです。波線が出ていると「へ〜、こういうことも気をつけた方がいいんだ」ということがわかります。

知らないことは調べないといけませんが必要なのことのはず。「何をしたらいいか」「何を知っておくといいか」といったことを教えてくれる。これが IDE を使うメリットだと僕は思います。

**だから、初心者にこそ JetBrains の IDE はおすすめできますね。（C/C++は特に）**
